...
export default function PostListItem({ post }: Props) {
  ...
  return (
    <Link href={href} className="group block py-5 border-t border-gray-100">
      <div className="sm:hidden w-full aspect-video overflow-hidden bg-gray-100 mb-3">
        {post.thumbnail ? (
          <Image
            src={post.thumbnail}
            alt={post.title}
      ...
      <div className="flex items-start gap-4 sm:gap-5">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="mt-1 text-base sm:text-lg font-semibold text-gray-900 leading-snug group-hover:text-gray-500 transition-colors line-clamp-2">
            {post.title}
          </h3>
        ...
        <div className="hidden sm:block w-44 md:w-52 shrink-0 self-start overflow-hidden bg-gray-100">
          {post.thumbnail ? (
            <Image
              src={post.thumbnail}
              alt={post.title}
