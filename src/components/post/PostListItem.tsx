...
function ThumbnailPlaceholder({ category }: { category: string }) {
  return (
    <div className="relative aspect-video w-full bg-linear-to-br from-gray-100 via-gray-150 to-gray-200 flex items-center justify-center">
...
export default function PostListItem({ post }: Props) {
  ...
  return (
    <Link href={href} className="group flex gap-4 sm:gap-5 py-5 border-t border-gray-100">
      <div className="flex-1 min-w-0">
        <span className="text-xs font-semibold tracking-widest uppercase text-gray-400">
          {post.category}
        </span>
        <h3 className="mt-1 text-lg font-semibold text-gray-900 leading-snug group-hover:text-gray-500 transition-colors line-clamp-2">
          {post.title}
        </h3>
