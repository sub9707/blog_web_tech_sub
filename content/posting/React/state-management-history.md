...
상태 관리의 역사는 결국 하나의 질문에 대한 끊임없는 탐색이다.

**변화를 어떻게 다룰 것인가.**

전역 변수로 시작해서, 이벤트 시스템을 거쳐, 단방향 데이터 흐름을 발견하고, 서버 상태와 클라이언트 상태를 분리하는 데 이르기까지. 각 시대의 해답은 그 시대의 문제를 풀었지만, 동시에 다음 시대의 문제를 만들었다.

좋은 개발자는 지금 쓰는 도구의 역사를 안다. 왜 이것이 만들어졌는지, 어떤 문제를 풀려 했는지, 어떤 한계가 있는지. 그것을 알아야 새로운 문제를 마주했을 때 올바른 선택을 할 수 있다.

그리고 기억해야 할 가장 중요한 것: **모든 추상화는 그것이 해결하는 문제만큼의 비용을 가진다.** 상태 관리 라이브러리를 추가하기 전에 항상 물어보라. 이 복잡성이 정말 필요한가. 더 단순한 방법은 없는가.

때로는 `useState` 하나가 가장 좋은 답이다.
