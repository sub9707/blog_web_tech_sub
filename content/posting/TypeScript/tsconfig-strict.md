...
thumbnail: "/assets/thumbnails/tsconfig-strict.png"
---
...
`strict`는 하나의 옵션이 아니다. 여러 엄격 검사 옵션을 한 번에 켜는 플래그다. 
...
![strict 플래그 분해 다이어그램](/assets/typescript/strict-setting.png)
...
배열이나 객체에서 값을 꺼낼 때마다 `undefined` 가능성이 생긴다. 조금 번거롭지만 실제로 안전하다.
...
`theme?`는 "없어도 된다"는 뜻이지, "undefined를 넣어도 된다"는 뜻이 아니다. 이 옵션이 그 차이를 강제한다.
...
`noImplicitAny`부터 켜고 `any`를 하나씩 제거한다. 그 다음 `strictNullChecks`로 null 관련 에러를 잡는다. 마지막에 `strict: true`로 나머지를 전부 켠다.
...
단점은 라이브러리 타입 오류를 못 잡는다. 하지만 대부분의 실용적인 프로젝트에서는 켜두는 게 낫다. 내가 쓰는 라이브러리 소스가 잘못된 건 내가 고칠 수 없으니까.
...
`strict: true`는 출발점이다. 거기서 `noUncheckedIndexedAccess`랑 `noImplicitReturns` 정도 더 켜두면 대부분의 실수를 컴파일 타임에 잡을 수 있다.
