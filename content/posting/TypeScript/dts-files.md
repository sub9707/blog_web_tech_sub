...
이걸 읽을 수 있다면 라이브러리 문서보다 더 정확한 정보를 볼 수 있을 것이다.
...
<!-- 이미지: axios 타입 구조 흐름
     - axios.get<User>() 호출에서 제네릭이 전달되는 흐름
     - AxiosInstance.get → T = User → AxiosResponse<User> → response.data: User
     - 단계별로 타입이 전파되는 과정을 화살표로 표현 -->
...
<!-- 이미지: 선언 병합을 통한 전역 타입 확장 구조
     - 왼쪽: 라이브러리 원본 타입 (Express.Request)
     - 중앙: 커스텀 .d.ts 파일의 선언 병합
     - 오른쪽: 병합 결과 (user 프로퍼티가 추가된 Request)
     - TypeScript 컴파일러가 두 선언을 합치는 흐름 표시 -->
...
`.d.ts`는 어렵지 않다. `declare`는 구현 없이 선언만 한다는 뜻이고, 나머지는 그냥 TypeScript 문법이다.

라이브러리 타입이 이해 안 될 때 바로 파일을 열어서 보는 습관을 들이면 구글링 시간이 많이 줄어든다.
