...
<!-- 썸네일 이미지: 코드 에디터 화면 절반, AI 채팅/에이전트 패널 절반으로 나뉜 심플한 일러스트. 텍스트 거의 없이 아이콘 위주 -->
...
<!-- 이미지: 자동완성형 / 채팅형 / 에이전트형 세 도구 카테고리를 화살표로 좌→우 진화 순서처럼 배치한 도식. 텍스트는 카테고리명만 -->
...
<!-- 이미지: 세 도구를 나란히 놓고 실행 환경 차이를 표현한 도식. 로컬 터미널 아이콘(Claude Code) / 클라우드 박스 아이콘(Codex) / 에디터+브라우저+터미널이 합쳐진 아이콘(Antigravity) -->
...
<!-- 이미지: 저울 형태로 왼쪽엔 "느림·정확·비쌈"이 쓰인 reasoning 모델, 오른쪽엔 "빠름·저렴"이 쓰인 경량 모델을 배치한 간단한 비교 그림 -->
...
<!-- 이미지: 컨텍스트 윈도우를 물이 차오르는 컵으로 비유한 그림. 컵 안에 대화/코드/파일이 쌓이다가 넘치는 순간을 표현 -->
...
<!-- 이미지: 여러 서비스(깃허브, 슬랙, 파일시스템, DB)가 각각 다른 모양의 플러그로 AI에 억지로 연결된 그림(Before) vs 하나의 통일된 MCP 단자로 깔끔하게 연결된 그림(After) -->
...
<!-- 이미지: 사용자 질문 → 벡터 DB에서 관련 문서 검색 → 검색된 조각을 프롬프트에 첨부 → LLM 응답, 4단계 흐름을 화살표로 표현한 도식. 텍스트는 단계명만 -->
...
<!-- 이미지: 하나의 저장소에서 git worktree로 분리된 3개의 작업 디렉터리, 각각에 에이전트 아이콘이 붙어 동시에 작업하는 모습을 표현한 도식 -->
