...
![행사장에 설치된 360도 촬영 키오스크 정면 — 회전 촬영대와 세로형 키오스크 화면](/assets/projects/360-kiosk/kiosk-front.webp)
...
![객석에서 바라본 행사장 부스 — 360° PHOTO ZONE 안내 월과 그 옆에 놓인 세로형 키오스크](/assets/projects/360-kiosk/photo-zone.webp)

## 회고

개발 환경에서는 보이지 않던 문제가 현장에서 고객 경험을 좌우한다는 것을 경험했습니다.

이 시스템은 안드로이드 앱에서 촬영한 영상을 키오스크로 전송한 뒤, FFmpeg로 배속·배경음악·인트로를 합성하고 Google Drive에 업로드한 다음 QR 코드로 전달하는 흐름입니다. 개발 환경과 회사의 빠른 네트워크에서는 각 단계가 무리 없이 처리됐지만, 행사장에서 제공되는 네트워크는 속도가 낮아 영상 전송과 업로드 구간의 대기 시간이 누적됐습니다. 현장에서 체험자와 사측은 "쓸 만한 속도"라고 평가했지만, 사용자 입장에서는 답답했을 것이라고 생각합니다.

클라이언트 환경에서 사용할 수 있는 저장소가 제한되어 Google Drive를 선택했는데, 대용량 미디어를 여러 단계에 걸쳐 처리할 때는 저장소 선택뿐 아니라 각 구간의 전송 시간이 전체 체감 속도를 좌우한다는 것을 알게 됐습니다. 이후 현장에 설치되는 프로젝트에서는 개발 전에 장비와 네트워크 조건을 먼저 확인하고 구조를 결정했습니다.
